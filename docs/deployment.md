...
Do not configure `GEMINI_API_KEY` or `GEMINI_MODEL` for a hosted Web service. Hosted mode intentionally ignores both variables; each user enters a key in the UI, and the built-in model is the initial selection. `YTTEXT_SUMMARY_LANG` and `YTTEXT_TRANSCRIPT_FORMAT` are independent of Gemini credentials and may be set in either local or hosted mode to choose the browser app's initial summary language and transcript format.
...
| `YTTEXT_TRANSCRIPT_FORMAT` | `md` | Initial browser transcript format and CLI default; accepts `md`, `txt`, `json`, `srt`, or `vtt` |
